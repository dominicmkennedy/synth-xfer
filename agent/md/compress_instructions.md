...
- Before making any substitution: call `get_target_file()` to fetch the transfer function, `get_library_text()` to fetch the available helpers, and `get_available_primitives()` to confirm allowed operations. For each library function, identify whether any contiguous sub-sequence of operations in the target computes exactly the same value with the same operands.
